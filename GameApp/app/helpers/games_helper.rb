module GamesHelper
  ...
  def is_player_playing game, user
    moves = game.tictactoe_moves.map {|x|x.move_square}
    puts moves.inspect
    puts game.playings.first.user_id == user.id
    return true if moves.empty? and game.playings.first.user_id == user.id
    @last_player_id=game.tictactoe_moves.last.player_position
     return @last_player_id == user.id
  end
